                ...
                match client.list_sessions().await {
                    Ok(sessions_list) => {
                        ...
                        if let Err(e) = cache.write_aliases(&aliases) {
                            log::error!("Failed to write aliases to cache: {:?}", e);
                        }
...
async fn default_message_handler(
    ...
    if let Some(text) = msg.text() {
        if let Some(client) = &*client.lock().await {
            let (identifier, message) = if let Some((id, msg)) = text.split_once(':') {
                (Some(id.trim()), msg.trim())
            } else {
                (None, text)
            };

            let session_id_result = if let Some(id) = identifier {
                client.list_sessions().await.map_err(|e| e.to_string()).and_then(|sessions| {
                    resolve_session_identifier(id, &sessions)
                })
            } else {
                match cache.read_current_session() {
                    Ok(Some(session_id)) => Ok(session_id),
                    Ok(None) => Err("No current session is set. Use /s <session_id_or_alias> to set one, or provide an identifier.".to_string()),
                    Err(e) => Err(e),
                }
            };
                        ...
                        if last_seen_activity_id.as_deref() != Some(&last_activity.id) {
                            ...
                            let notification_message = if let Some(agent_messaged) = &last_activity.agent_messaged {
                                ...
                                Some(format!(
                                    ...
                                    escape_markdown_v2(&session_display)
                                ))
